...
4.  **Set Up MongoDB Connection**:
    ...
    - Add your MongoDB connection string to this file. Replace the placeholder with your actual URI.
      ...
      // backend/config.js
      module.exports = {
        mongoURI: "mongodb://192.168.1.155:27017/geolocation_tracker", // Replace with your MongoDB URI (e.g., from MongoDB Atlas)
      };
      ```
...
5.  **Run the Backend Server**:
    ...
    npm start
    ```
    The backend server should now be running, typically on `http://192.168.1.155:5000` (check `server.js` for the exact port).
...
3.  **Configure Backend API URL**:
    ...
    - Check files like `web-ui/src/App.js` or `web-ui/src/services/api.js` for the API base URL. Ensure it points to your backend server (e.g., `http://192.168.1.155:5000`).
4.  **Run the Web UI**:
    ```bash
    npm start
    ```
    The web UI should automatically open in your browser, typically at `http://192.168.1.155:3000`.
